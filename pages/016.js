...
<p className="it">Los componentes controlados son resistentes. La característica de definición de un componente controlado es que el valor mostrado está enlazado al estado del componente; para actualizar el valor que ejecuta una función adjunta al controlador <code>onChange</code> de eventos en el elemento de formulario. La función <code>onChange</code> actualiza la propiedad de estado, que a su vez actualiza el valor del elemento de formulario.</p>
...
<li className="it">Puede <strong>renderizar dinámicamente otros componentes en</strong> función del valor del componente controlado. Por ejemplo, el valor que el usuario selecciona en una lista desplegable (por ejemplo, 'perro' o 'gato') puede controlar qué otros componentes de formulario (por ejemplo, un conjunto de casillas de razas) se representan en el formulario.</li>
...
<p className="it">La desventaja de los componentes controlados es la cantidad de código que tiene que escribir. Necesita una propiedad de estado para pasar al elemento de formulario como <code>props</code>, y necesita una función para actualizar el valor de esta propiedad.</p>
...
<p className="it">Una manera más fácil y menos laboriosa de tomar valores de un elemento de formulario es usar la propiedad <code>ref</code>. Diferentes elementos de forma y composición de componentes requieren diferentes estrategias, por lo que el resto de este post se divide en las siguientes secciones.</p>
...
<ol>
<li className="it"><strong>Entradas de texto y números, selecciona</strong>  </li>

<li><strong>Text &amp; number inputs, selects</strong>  </li>
<li className="it"><strong>Paso de accesorios del niño al padre</strong>  </li>

<li><strong>Passing props from child to parent</strong>  </li>
<li className="it"><strong>Conjuntos de radio</strong>  </li>

<li><strong>Radio sets</strong>  </li>
<li className="it"><strong>Conjuntos de casillas de verificación</strong></li>
...
<p className="it">Las entradas de texto y números proporcionan el ejemplo más sencillo de usar <code>ref</code>s. En el atributo <code>ref</code> de la entrada, agregue una arrow function que tome la entrada como un argumento. Tiendo a nombrar el argumento de la misma manera que el elemento en sí, como se ve a continuación en la línea 3:</p>
...
<p className="it">A continuación, toma el argumento y asignarlo a una propiedad asociada a la palabra clave <code>this</code> de la clase . Las entradas (es decir, el nodo DOM) están ahora accesibles como <code>this.fullName</code> y <code>this.amount</code>. Los valores de las entradas son accesibles como <code>this.fullName.value</code> y <code>this.amount.value</code>.</p>

<p>You then take the argument and assign it to a property attached to the class's <code>this</code> keyword. The inputs (i.e. the DOM node) are now accessible as <code>this.fullName</code> and <code>this.amount</code>. The values of the inputs are accessible as <code>this.fullName.value</code> and <code>this.amount.value</code>.</p>
...
<p className="it">El valor seleccionado es accesible como <code>this.petType.value</code>.</p>

<p>The value selected is accessible as <code>this.petType.value</code>.</p>

<h4 id="2passingpropsfromchildtoparent">2. Passing props from child to parent</h4>

<p className="it">Con un componente controlado, obtener el valor de un componente hijo a un padre es sencillo - el valor ya vive en el padre! Se transmite al niño. También una función <code>onChange</code> se pasa y se actualiza el valor a medida que el usuario interactúa con la interfaz de usuario.  </p>
...
<p className="it">Mientras que el valor ya vive en el estado de los padres en los componentes controlados, esto no ocurre cuando se usa <code>ref</code>s. Con <code>ref</code>s, el valor reside en el nodo DOM en sí mismo, y debe ser comunicada <em>arriba</em> al parent.</p>
...
<p className="it">Para pasar este valor de niño a padre, el padre necesita pasar un <em>"gancho"</em>, si lo desea, al niño. El niño entonces agrega un nodo al 'gancho' para que el padre tenga acceso a él.</p>
